...
export const register = (formData) => axios.post('/api/users/v1/register', formData)
// auth.js
...
//       const { data } = await A.register(formData);
//       // Call the login function only if user registration is successful
//       await loginIn(formData, setShowAlert, setAlertMessage);
//       return data;
//   } catch (err) {
//       setShowAlert(true);
//       if (err.response && (err.response.status === 400 || err.response.status === 401)) {
//           // Display specific error message if available in the response
//           setAlertMessage(err.response.data.message || 'Authentication failed');
//       } else {
